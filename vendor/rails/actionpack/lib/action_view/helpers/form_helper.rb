  ...
  module Helpers
    ...
    class InstanceTag #:nodoc:
      ...
      def to_check_box_tag(options = {}, checked_value = "1", unchecked_value = "0")
        ...
        options["checked"] = "checked" if checked
        add_default_name_and_id(options)
        # See http://groups.google.com/group/rubyonrails-core/browse_thread/thread/b6295ea961826355.
        cb = tag("input", options)
        unless options['do_not_generate_hidden_field_see_fxn_hack_in_vendor_form_helper_rb']
           cb << tag("input", "name" => options["name"], "type" => "hidden", "value" => options['disabled'] && checked ? checked_value : unchecked_value)
        end
        cb
      end
